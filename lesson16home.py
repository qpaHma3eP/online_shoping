...
def send_welcome(message):
    ...
    bot.send_message(chat_id, "Добро пожаловать! Пожалуйста, зарегистрируйтесь, отправив свой номер телефона.",
                     reply_markup=markup)
...
def contact_handler(message):
    ...
    if message.contact is not None:
        ...
        cursor.execute("INSERT INTO users (user_id, first_name, phone_number) VALUES (?, ?, ?)",
                       (user_id, first_name, phone_number))
        conn.commit()

        markup = types.ReplyKeyboardMarkup(one_time_keyboard=True, resize_keyboard=True)
        markup.add(types.KeyboardButton('Отправить местоположение📍', request_location=True))
        bot.send_message(chat_id, "Спасибо! Ваши данные сохранены. Теперь, пожалуйста, отправьте ваше местоположение.",
                         reply_markup=markup)
...
# Ваш Telegram-канал
TELEGRAM_CHANNEL = 'https://t.me/YOUR_TELEGRAM_CHANNEL'

# Ваш Instagram-профиль
INSTAGRAM_PROFILE = 'https://www.instagram.com/YOUR_INSTAGRAM/'
...
def show_main_menu(chat_id, language):
    ...
    if language == 'ru':
        markup.add(types.KeyboardButton('Отправить номер телефона', request_contact=True))
        markup.add(types.KeyboardButton('Показать справочную информацию'))
    elif language == 'uz':
        markup.add(types.KeyboardButton('Telefon raqamni yuborish', request_contact=True))
        markup.add(types.KeyboardButton('Ma\'lumotnoma ko\'rsatish'))
    bot.send_message(chat_id, "Главное меню / Asosiy menyu:", reply_markup=markup)
...
# Обработка сообщений из меню
@bot.message_handler(func=lambda message: True)
def echo_all(message):
    chat_id = message.chat.id
    if message.text == 'Показать справочную информацию' or message.text == 'Ma\'lumotnoma ko\'rsatish':
        send_help(message)
        return
