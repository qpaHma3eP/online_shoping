...
def get_exact_price(pr_name):
    return sql.execute('SELECT pr_products FROM products WHERE pr_name=?;',(pr_name,)).fetchone()
...
def make_order(user_id):
    # Достаем названия товаров и их количество с корзины
    ...
    # Достаем количество продуктов со склада
    stock_quantity = [sql.execute('SELECT pr_count FROM products WHERE pr_name=?;', (i[0],)).fetchone() for i in product_names]
    totals = []

    # е - сколько взял пользователь
    ...
    for t in totals:
        ...
        for n in product_names:
            sql.execute('UPDATE products SET pr_count=?;', (t, n[0]))
# Фиксируем изменения
    connection.commit()
...
# роверка на наличие товаров в БД
def check_pr():
